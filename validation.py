...
class PlaylistValidation(DataValidation):
    ...
    def is_valid_playlist(playlist_data):
        ...
        return all([PlaylistValidation.is_valid_alpha(input_playlist_name),
                    PlaylistValidation.is_valid_alpha(input_description),
                    PlaylistValidation.is_valid_username(input_username),
                    PlaylistValidation.is_valid_email(email)])

class TrackValidation(DataValidation):
    ...
    def is_valid_track(track):
        ...
        return all([TrackValidation.is_valid_special_character(input_track_name),
                    TrackValidation.is_valid_alpha(input_artist),
                    TrackValidation.is_valid_special_character(input_album_name),
                    TrackValidation.is_valid_date(input_date),
                    TrackValidation.is_valid_integer(input_durationtime),
                    TrackValidation.is_valid_integer(input_popularity)])
...
class SpotifyValidation:
    ...
    def is_valid_json(spotify_json):
        ...
        for playlist_data in spotify_json:
            if PlaylistValidation.is_valid_playlist(playlist_data):
                for track in playlist_data['tracks']:
                    if TrackValidation.is_valid_track(track):
                        return True
                    else:
